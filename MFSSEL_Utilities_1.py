...
def get_confidence_index(probilityList):
    ...
    for i in hog_svc_probility:
        hog_svc_confidence_list.append(get_confidence(i, 0.3))
    for i in _81_svc_probility:
        _81_svc_confidence_list.append(get_confidence(i, 0.3))
    for i in _30_svc_probility:
        _30_svc_confidence_list.append(get_confidence(i, 0.3))
...
def vote(predict_Y_list, real_unlabeled_Y):
    ...
    for i in range(len(hog_svc_predict_Y)):
        ...
        if (hog_svc_predict_Y[i] == _81_svc_predict_Y[i]):
            ...
            # voted_predict_Y_list.append(hog_unlabeled_Y[i])
            continue
        if (hog_svc_predict_Y[i] == _30_svc_predict_Y[i]):
            voted_index_result.append(i)
            voted_predict_Y_list.append(hog_svc_predict_Y[i])
            # voted_predict_Y_list.append(hog_unlabeled_Y[i])
            continue
        if (_81_svc_predict_Y[i] == _30_svc_predict_Y[i]):
            voted_index_result.append(i)
...
def get_topk_Y_index(c, topk, voted_predict_Y_list, voted_index_result, hog_svc_confidence_list,
    ...
    for i in range(len(voted_predict_Y_list)):
        if voted_predict_Y_list[i] == c:
            ...
            avg_confidence = 0.98 * (hog_svc_confidence_list[i]) + 0.01 * (_81_svc_confidence_list[i]) + 0.01 * (
...
def get_max_probility(probilities):
    if np.size(probilities) == 1:
        confidence = 1
    else:
        probilities = np.array(probilities)
...
def get_confidence_index_1(probility_list, predict_Y_list, unlabeled_Y_list):
    """
    获取SVM置信度较高的索引
    :param probilityList:三个分类器对应的每个类别的概率
    :return:
    """
    # 每个类别的概率
    hog_probility = probility_list[0]
    _81_probility = probility_list[1]
    _30_probility = probility_list[2]
    # 每个类别的预测结果
    hog_predict_Y = predict_Y_list[0]
    _81_predict_Y = predict_Y_list[1]
    _30_predict_Y = predict_Y_list[2]
    # 每个类别的预测结果
    hog_unlabeled_Y = unlabeled_Y_list[0]
    _81_unlabeled_Y = unlabeled_Y_list[1]
    _30_unlabeled_Y = unlabeled_Y_list[2]

    hog_unlabeled_Y = np.array(hog_unlabeled_Y)
    _81_unlabeled_Y = np.array(_81_unlabeled_Y)
    _30_unlabeled_Y = np.array(_30_unlabeled_Y)

    max_hog_probility = []
    max_81_probility = []
    max_30_probility = []

    for i in hog_probility:
        max_hog_probility.append(get_max_probility(i))
    for i in _81_probility:
        max_81_probility.append(get_max_probility(i))
    for i in _30_probility:
        max_30_probility.append(get_max_probility(i))
    max_hog_probility = np.array(max_hog_probility)
    max_81_probility = np.array(max_81_probility)
    max_30_probility = np.array(max_30_probility)

    ind_max_hog_probility = np.argsort(-max_hog_probility)
    ind_max_81_probility = np.argsort(-max_81_probility)
    ind_max_30_probility = np.argsort(-max_30_probility)

    sorted_max_hog_probility = max_hog_probility[ind_max_hog_probility]
    sorted_max_81_probility = max_81_probility[ind_max_81_probility]
    sorted_max_30_probility = max_30_probility[ind_max_30_probility]
    ...
    # # 添加置信度list
    # for i in hog_probility:
    #     hog_confidence_list.append(get_confidence(i, 0.1))
    # for i in _81_probility:
    #     _81_confidence_list.append(get_confidence(i, 0.1))
    # for i in _30_probility:
    #     _30_confidence_list.append(get_confidence(i, 0.1))
    1 == 1
