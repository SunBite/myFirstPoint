...
class DataPreparation:
    ...
    def getLabelAndNameTupleList(self, path):
        ...
        unlabeled_x, labeled_x, unlabeled_y, labeled_y = train_test_split(trainnameList, trainlabelList,
                                                                          test_size=0.54,
                                                                          stratify=trainlabelList,
                                                                          random_state=2)  # 2 0.2 0.36 0.54
    ...
    def getLabelAndNameTupleList_KFold(self, path):
        """
        遍历文件夹下所有的feature，获取到split之后的名字列,得到相应的标签
        :param path:文件夹路径
        :return: 有标签数据的标签和名字Tuplelist，无标签数据的标签和名字Tuplelist，测试集数据的标签和名字Tuplelist
        ...
        for classname in classnames:
            featurePath = path + classname + os.sep + "hogvideoFeature"
            # 遍历文件夹
            for dirpath, dirnames, filenames in os.walk(featurePath):
                for filename in filenames:
                    labelIndex = self.__classmap[filename.split("_v")[0]]
        ...
        for train_index, test_index in sfolder.split(nameList, labelList):
            X_train, X_test = nameList[train_index], nameList[test_index]
            Y_train, Y_test = labelList[train_index], labelList[test_index]

            # 训练集和测试集比例为9：1
            unlabeled_x, labeled_x, unlabeled_y, labeled_y = train_test_split(X_train, Y_train,
                                                                              test_size=0.54,
                                                                              stratify=Y_train,
                                                                              random_state=2)# 0.2 0.81  0.36 0.8 0.54 0.7
            # 有标签数据的标签和名字的Tuplelist
            Label_Name_Labeled_train_tuple_list = self.__get_Y_X_tuple_list(labeled_y, labeled_x)
            # 无标签数据的标签和名字Tuplelist
            Label_Name_Unlabeled_train_tuple_list = self.__get_Y_X_tuple_list(unlabeled_y, unlabeled_x)
            # 测试集数据的标签和名字Tuplelist
            Label_Name_test_tuple_list = self.__get_Y_X_tuple_list(Y_test, X_test)
            train_test_tuple_list.append((Label_Name_Labeled_train_tuple_list, Label_Name_Unlabeled_train_tuple_list,
                               Label_Name_test_tuple_list))
